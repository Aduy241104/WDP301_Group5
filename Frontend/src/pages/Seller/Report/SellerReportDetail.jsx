...
import {
  getSellerReportDetailAPI,
  getSellerShopReportDetailAPI,
...
export default function SellerReportDetail() {
  ...
  const isShopReport = location.pathname.includes("shop-reports");
  ...
  const fetchReport = async () => {
    try {
      ...
      if (isShopReport) {
        data = await getSellerShopReportDetailAPI(reportId);
      } else {
        data = await getSellerReportDetailAPI(reportId);
      }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow p-6 space-y-5">
          <div>
            <div className="text-sm text-slate-500">
              {report.targetType === "shop" ? "Shop" : "Product"}
            </div>
        ...
        <div className="bg-white rounded-xl shadow p-6 space-y-5">
          <div>
            <div className="text-sm text-slate-500">Customer Reason</div>
            <div className="font-medium">{report.reason}</div>
          </div>

          {report.description && (
            <div>
              <div className="text-sm text-slate-500">Customer Description</div>
          ...
          {report.adminMessage && (
            <div>
              <div className="text-sm text-slate-500">Admin Response</div>
      ...
      {report.images?.length > 0 && (
        <div className="bg-white rounded-xl shadow p-6">
          <div className="text-sm text-slate-500 mb-4">Evidence Images</div>
