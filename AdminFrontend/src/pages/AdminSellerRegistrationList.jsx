...
function getUserIdFromRequest(item) {
...
function StatusPill({ status }) {
    const map = {
        ...
        rejected: { label: "Từ chối", cls: "bg-slate-100 text-slate-700" },
...
export default function AdminSellerRegistrationList() {
    ...
    const refreshData = async () => {
        try {
            ...
            const [reqRes, shopRes] = await Promise.all([
                fetchSellerRegistrations({ page: 1, limit }),
    ...
    const baseItems = useMemo(() => {
        ...
        return allItems.filter((it) => getDisplayStatus(it) === statusFilter);
    ...
    const stats = useMemo(() => {
        ...
        const active = allItems.filter((it) => getDisplayStatus(it) === "active").length;
        const pending = allItems.filter((it) => getDisplayStatus(it) === "pending").length;
        const blocked = allItems.filter((it) => getDisplayStatus(it) === "blocked").length;
    ...
    return (
        <div className="space-y-6">
            {/* Title */}
            <div>
                <h1 className="text-2xl font-bold text-slate-900">Quản lý Người bán</h1>
                <p className="text-slate-500 mt-1">Danh sách và quản lý người bán trên hệ thống</p>
            </div>
                    ...
                    <table className="min-w-full text-sm">
                        <thead className="bg-slate-50 text-slate-600">
                            <tr className="border-b border-slate-200">
                                ...
                                <th className="px-5 py-3 text-center font-semibold">Số shop</th>
                                <th className="px-5 py-3 text-center font-semibold">Sản phẩm</th>
